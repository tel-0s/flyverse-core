...
submission and applied by `analyse` without a tolerance: a pair either repeats exactly or it does not.
    ...
    PYTHONIOENCODING=utf-8 python scripts/determinism_gate.py plan --out out/det1 [--ship flyverse]
        -> out/det1/batch.sh (DRAFT: `set -o pipefail`, ONE cluster_run.py call of 5 jobs, the docs/INTERP.md 10.3 job
           ...
           --ship PATH[,PATH] adds cluster_run.py --ship to the generated call, so that the committed plan reproduces
           a submission made against a target checkout behind origin/main (section 2 of the audit).
...
def plan(out_dir: Path, seconds: float = 10.0, minutes: int = 40, name: str = "det1", ship: str | None = None):
    ...
    ship = (ship or "").strip() or None
    if ship is not None and (ship.startswith("-") or any(c.isspace() for c in ship)):
        raise ValueError("--ship takes PATH[,PATH] with no spaces")
    ...
    lines = ["#!/bin/bash", "set -o pipefail",
             ...
             ] + ([f"# --ship {ship}: the named tracked paths are copied to the target whether or not they differ from "
                   "origin/main (a target checkout behind origin/main otherwise runs its own stale copy)."] if ship else []) + [
             ...
             f"--gpu-ids {GPU_POOL} " + (f"--ship {ship} " if ship else "") + f"--name {name} --minutes {minutes} "
...
def main():
    ...
    p.add_argument("--ship", default=None, help="cluster_run.py --ship PATH[,PATH] for the generated call")
    ...
    a = ap.parse_args()
    if a.cmd == "plan":
        plan(Path(a.out), a.seconds, a.minutes, a.name, a.ship)

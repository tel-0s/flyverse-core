"""Per-family heimdall job counts on a rented box, over plain ssh (no tunnel).
...
def main():
    ...
    ssh = ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=30", "-o", "StrictHostKeyChecking=no", "-p", str(t["port"]), t["ssh"], "python3 - '" + a.prefix + "'"]
    while True:
        try:

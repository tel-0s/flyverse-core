...
# ------------------------------------------------------------------------------------------------ the room rate-half
ROOM_RUNS = list(range(6))                       # brain seed k, env seeds 16k .. 16k+15 (guard_suites.sh's seed-matched batches, extended to 6)
ROOM_ARGS = "--batch 16 --minutes 5 --energy 0.9 --program cx --fruit apple --fence --cuda-graphs --cuda-kernels --event-driven --cuda-sparse torch"
ROOM_FLY_S = 16 * 300.0


def room_command(arm: str, k: int, rel: str) -> tuple[str, str]:
    stem = f"{rel}/room_{arm}_r{k}"
    seeds = ",".join(str(s) for s in range(16 * k, 16 * k + 16))
    return (f"PYTHONIOENCODING=utf-8 python scripts/instrumented_room.py --arm {arm} --json {stem}.json -- {ROOM_ARGS} --seed {k} --seeds {seeds} "
            f"--log-every 60"), stem


def plan_room(out_dir: Path, minutes: int = 120, name: str = "suite-inst-room"):
    out_dir = Path(out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    rel = f"out/{out_dir.name}"
    if (out_dir / "predeclared.json").exists():
        raise ValueError("batch is frozen; use a new directory for a new predeclaration")
    jobs = []
    for k in ROOM_RUNS:
        for arm in ("raw", "instrumented"):
            cmd, stem = room_command(arm, k, rel)
            jobs.append(dict(arm=arm, run=k, brain_seed=k, env_seeds=list(range(16 * k, 16 * k + 16)), stem=stem, command=cmd,
                             line=(f"mkdir -p {rel} && source .venv/bin/activate && python -c 'import torch; assert torch.cuda.is_available()' && "
                                   f"{cmd} > {stem}.txt 2>&1; st=\\$?; test -s {stem}.json || st=1; tail -3 {stem}.txt; exit \\$st")))
    stamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    lines = ["#!/bin/bash", "set -o pipefail",
             f"# DRAFT -- NOT SUBMITTED. Round 8 / batch {name}: docs/audits/instrumented_suite.md section 3 (the room rate-half).",
             f"# {len(jobs)} jobs = 2 arms x {len(ROOM_RUNS)} seed-matched runs of batch_sustain.py's take-off protocol ({ROOM_ARGS}), 16 rooms x 300 s",
             f"# = {ROOM_FLY_S:,.0f} fly-s per run, one cluster_run.py call; every job pinned to one GPU of the pool {GPU_POOL}. The second cluster node is",
             "# named by the CLUSTER_NODE_2 environment variable (docs/CLUSTER.md, git-ignored); committed text never carries the name.",
             "# Generated by scripts/instrumented_suite.py plan-room on " + stamp + ".",
             "python scripts/cluster_run.py --target house --node \"${CLUSTER_NODE_2:?set CLUSTER_NODE_2 to the name of the second cluster node}\" "
             f"--gpu-ids {GPU_POOL} --ship flyverse,scripts --name {name} --minutes {minutes} "
             + " ".join('"' + j["line"] + '"' for j in jobs) + f" --fetch {rel}/ 2>&1 | tee {rel}/client_stdout_0.txt || exit $?"]
    (out_dir / "batch.sh").write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    (out_dir / "jobs.json").write_text(json.dumps({"batch": name, "runs": ROOM_RUNS, "room_args": ROOM_ARGS, "fly_s_per_run": ROOM_FLY_S,
                                                   "instruments": INSTRUMENTS, "hold_edges": HOLD, "nt_override": NT_OVERRIDE, "record_names": RECORD_NAMES,
                                                   "gpu_pool": GPU_POOL, "node": "<cluster-node-2>", "jobs": jobs, "generated_utc": stamp,
                                                   "status": "DRAFT, not submitted"}, indent=1), encoding="utf-8")
    print(f"{len(jobs)} jobs in 1 call -> {out_dir / 'batch.sh'} (DRAFT, not submitted); {out_dir / 'jobs.json'}")


def predeclare_room(out_dir: Path):
    out_dir = Path(out_dir)
    if list(out_dir.glob("room_*_r?.json")):
        raise ValueError("cannot predeclare after results exist")
    plan_rec = json.loads((out_dir / "jobs.json").read_text(encoding="utf-8"))
    hashes = source_hashes()
    for extra in ("scripts/instrumented_room.py", "scripts/batch_sustain.py"):
        p = ROOT / extra
        hashes[extra] = hashlib.sha256(p.read_bytes().replace(b"\r\n", b"\n")).hexdigest()
    record = dict(plan_rec, status="PREDECLARED, not submitted", written_before_submission=True,
                  stamped_utc=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                  batch_sha256=hashlib.sha256((out_dir / "batch.sh").read_bytes()).hexdigest(),
                  jobs_sha256=hashlib.sha256((out_dir / "jobs.json").read_bytes()).hexdigest(),
                  source_sha256_lf=hashes,
                  rule={"replicate_unit": "runs: one 16-room x 300 s batch per job, six per arm, seed-matched (brain seed k, env seeds 16k..16k+15)",
                        "primary": "total take-offs (hops = escape + voluntary) per run at equal exposure (4,800 fly-s): a two-sample exact Poisson "
                                   "(conditional binomial on the pooled counts, scipy.stats.binomtest(K_inst, K_inst + K_raw, 0.5)), ONE-SIDED, "
                                   "direction stated once: the instrumented list is `worse` iff its take-off rate is HIGHER (H1: instrumented > raw); "
                                   "fail at p < 0.05 (INTERP 10.4 rule 17); the run-level common.compare (6 v 6) is quoted beside it",
                        "descriptive": "escape and voluntary take-offs separately (the same tests, no verdict), meals, path, walking-GF median",
                        "validity": "12 runs; the wrapper's `room` block with no problems (raw: preset raw, no records, shipped cache; instrumented: the "
                                    "three records, the hold in type_path_gain, GLNO glutamate, the relabel cache); device cuda; 16 rows x 30,000 frames; "
                                    "every loaded source at the predeclared hash",
                        "adoption": "none either way: a pass records that the list is admissible under PRESETS_SPEC section 2 item 5 (suite half + room "
                                    "rate-half), nothing more"})
    with (out_dir / "predeclared.json").open("x", encoding="utf-8", newline="\n") as f:
        json.dump(record, f, indent=2)
        f.write("\n")
    print(f"froze {out_dir / 'predeclared.json'} before submission")
...
def analyse_room(runs_dir: Path, out_dir: Path):
    ...
    for k in ROOM_RUNS:
        for arm in ("raw", "instrumented"):
            p = runs_dir / f"room_{arm}_r{k}.json"
            if not p.is_file():
                problems.append(f"{p.name}: missing"); continue
            d = json.loads(p.read_text(encoding="utf-8"))
            rb, prov = d.get("room", {}), d.get("provenance", {})
            bad = list(rb.get("problems", []) or [])
            if rb.get("arm") != arm:
                bad.append(f"arm {rb.get('arm')!r}")
            if d.get("batch") != 16 or d.get("frames") != 30000 or d.get("brain_seed") != k:
                bad.append(f"batch {d.get('batch')} frames {d.get('frames')} seed {d.get('brain_seed')}")
            if [r.get("environment_seed") for r in d.get("rows", [])] != list(range(16 * k, 16 * k + 16)):
                bad.append("env seeds")
            if not str(prov.get("execution", {}).get("device", "")).startswith("cuda"):
                bad.append(f"device {prov.get('execution', {}).get('device')!r}")
            md5 = prov.get("compiled_connectome", {}).get("md5")
            if md5 != CACHE_MD5[arm]:
                bad.append(f"compiled cache {md5}")
            ...
            console = p.with_suffix(".txt")
            txt = console.read_text(encoding="utf-8", errors="replace") if console.is_file() else ""
            if "device cuda" not in txt:
                bad.append("console lacks 'device cuda'")
            if "Traceback" in txt:
                bad.append("console carries a Traceback")
            problems.extend(f"{p.name}: {b}" for b in bad)
            rows.append(dict(arm=arm, run=k, brain_seed=d.get("brain_seed"), file=p.name, device_name=rb.get("device_name"), gpu=rb.get("cuda_visible_devices"),
                             cache_md5=md5, hops=d.get("hops_total"), escape=d.get("hops_escape_total"), voluntary=d.get("hops_voluntary_total"),
                             fly_s=d.get("fly_s"), meals=sum(r.get("meals", 0) for r in d.get("rows", [])),
                             path_m=float(np.mean([r.get("path_m", np.nan) for r in d.get("rows", [])])), gf_walk_median_hz=d.get("gf_max_walk_median_hz"),
                             rows_gf_at_threshold=d.get("rows_gf_at_threshold"), wall_s=d.get("wall_s"), checks="; ".join(bad)))
    df = pd.DataFrame(rows); df.to_csv(out_dir / "room_runs.csv", index=False)
    tests = []
    for key, role in (("hops", "primary"), ("escape", "descriptive"), ("voluntary", "descriptive")):
        a = df[df.arm == "instrumented"][key].to_numpy(float); b = df[df.arm == "raw"][key].to_numpy(float)
        Ka, Kb = int(np.nansum(a)), int(np.nansum(b))
        Ta, Tb = float(df[df.arm == "instrumented"].fly_s.sum()), float(df[df.arm == "raw"].fly_s.sum())
        rec = dict(measure=key, role=role, K_instrumented=Ka, K_raw=Kb, T_instrumented_fly_s=Ta, T_raw_fly_s=Tb,
                   rate_instrumented_per_1000=Ka / Ta * 1000 if Ta else float("nan"), rate_raw_per_1000=Kb / Tb * 1000 if Tb else float("nan"),
                   instrumented_runs=json.dumps([int(x) for x in a]), raw_runs=json.dumps([int(x) for x in b]))
        if Ka + Kb > 0 and Ta == Tb:
            rec["p_exact_one_sided_instrumented_higher"] = float(binomtest(Ka, Ka + Kb, 0.5, alternative="greater").pvalue)
            rec["p_exact_two_sided"] = float(binomtest(Ka, Ka + Kb, 0.5, alternative="two-sided").pvalue)
        else:
            rec["p_exact_one_sided_instrumented_higher"] = float("nan") if Ta == Tb else float("nan")
            rec["p_exact_two_sided"] = float("nan")
            rec["note"] = "no take-off in either arm" if Ka + Kb == 0 else "unequal exposure"
        if len(a) and len(b):
            c = common.compare(a, b)
            rec.update(compare_verdict=c["verdict"], diff=c["diff"], z=c["z"], p_runs=c["p"], null_sd_zero=c["null_sd_zero"])
        tests.append(rec)
    pd.DataFrame(tests).to_csv(out_dir / "room_tests.csv", index=False)
    primary = tests[0]
    complete = len(df) == 12 and not problems
    p1 = primary.get("p_exact_one_sided_instrumented_higher", float("nan"))
    verdict = ("INCOMPLETE or invalid" if not complete else
               "no take-off in either arm: the rate-half is undetermined (nothing to compare)" if primary.get("note") == "no take-off in either arm" else
               f"FAIL: the instrumented take-off rate is higher (one-sided exact p {p1:.4g})" if p1 < 0.05 else
               f"PASS: the instrumented take-off rate is not higher (one-sided exact p {p1:.4g})")
    summary = dict(runs_dir=runs_dir.as_posix(), verdict=verdict, tests=tests, problems=problems, n_runs=len(df),
                   stamped_utc=frozen.get("stamped_utc"), analysis_sha256=hashlib.sha256(Path(__file__).read_bytes().replace(b"\r\n", b"\n")).hexdigest(),
                   created_utc=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
    (out_dir / "room_summary.json").write_text(json.dumps(summary, indent=1, default=str) + "\n", encoding="utf-8")
    md = [f"# suite-inst room rate-half analysis of `{runs_dir.as_posix()}`", "", f"problems {len(problems)}" + (": " + "; ".join(problems[:12]) if problems else ""), "",
          f"**Verdict: {verdict}**", "",
          ...
          "|---|---|---|---|---|---|---|---|---|---|"]
    for t in tests:
        md.append(f"| {t['measure']} | {t['role']} | {t['K_instrumented']} | {t['K_raw']} | {t['rate_instrumented_per_1000']:.3f} / {t['rate_raw_per_1000']:.3f} | "
                  f"{t.get('p_exact_one_sided_instrumented_higher', float('nan')):.4g} | {t.get('p_exact_two_sided', float('nan')):.4g} | "
                  f"{t.get('compare_verdict')} (diff {t.get('diff', float('nan')):+.3f}, z {t.get('z', float('nan')):+.2f}, p {t.get('p_runs', float('nan')):.4g}) | "
                  f"{t['instrumented_runs']} | {t['raw_runs']} |")
    md += ["", "| arm | run | brain seed | device | GPU | cache | hops | escape | voluntary | meals | path m | walking-GF median Hz | rows GF >= 33 | wall s | checks |",
           "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for r in rows:
        md.append(f"| {r['arm']} | {r['run']} | {r['brain_seed']} | {r['device_name']} | {r['gpu']} | {str(r['cache_md5'])[:8]} | {r['hops']} | {r['escape']} | {r['voluntary']} | "
                  f"{r['meals']} | {r['path_m']:.3f} | {r['gf_walk_median_hz']} | {r['rows_gf_at_threshold']} | {r['wall_s']} | {r['checks'] or 'ok'} |")
    (out_dir / "room_analysis.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    print("\n".join(md[:10]))
    print(f"-> {out_dir}")
    return summary
...
def main():
    ...
    p = sub.add_parser("plan-room"); p.add_argument("--out", default="out/suite-inst-room"); p.add_argument("--minutes", type=int, default=120); p.add_argument("--name", default="suite-inst-room")
    p = sub.add_parser("predeclare-room"); p.add_argument("dir")
    p = sub.add_parser("analyse-room"); p.add_argument("--runs", default="out/suite-inst-room"); p.add_argument("--out", default=None)
    a = ap.parse_args()
    if a.cmd == "plan":
        ...
        if s["problems"]:
            raise SystemExit(2)
    elif a.cmd == "plan-room":
        plan_room(Path(a.out), a.minutes, a.name)
    elif a.cmd == "predeclare-room":
        predeclare_room(Path(a.dir))
    else:
        s = analyse_room(Path(a.runs), Path(a.out or (Path(a.runs) / "analysis")))
        if s["problems"]:
            raise SystemExit(2)

...
def footprint(rad_obj: np.ndarray, rad_blank: np.ndarray, col_az_el: np.ndarray, dim_thresh: float = 0.5,
    ...
    radiance the object dims by more than `dim_thresh` (50 %), those it changes by more than `change_thresh` (5 %),
    the unweighted az / el centroid of the dimmed set, and the blank (background) luminance under it."""
    ...
    n_d = dimmed.sum(1); n_c = changed.sum(1); n_b = brightened.sum(1)
    ...
    return {"per_frame": {"n_dimmed_50pct": n_d, "n_changed_5pct": n_c, "n_brightened_50pct": n_b, "centroid_az_deg": cen_az,
                          "centroid_el_deg": cen_el, "el_min_deg": el_lo, "el_max_deg": el_hi, "blank_lum_under_object": bg,
                          "min_rel": rel.min(1) if T else np.zeros(0)},
...
def load_rf_map(path) -> pd.DataFrame:
    ...
    interface; the synthetic-stimuli task's own format (docs/INTERP.md) is read through the same columns."""
    path = Path(path)
...
def verify_runs(paths: list) -> tuple[pd.DataFrame, list]:
    ...
    for p in sorted(paths):
        ...
        row = {"run": p.stem, "diam_deg": pr.get("diam_deg"), "elevation_deg": pr.get("elevation_deg"), "null": bool(d.get("summary", {}).get("null")),
               "device": ex.get("device"), "retina_mode": prov.get("retina", {}).get("mode"), "console_device_cuda": None}
        if txt.exists():

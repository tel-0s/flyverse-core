...
class CachedSubsetTests(unittest.TestCase):
    ...
    def test_c_batch_configuration_has_no_inhibitory_entry_and_the_linear_sum_flips(self):
        """`verify:hooks`, pinned: on the arm the liveness batch actually ran (RECT = Mi1|Tm3|Tm2 -> T3 `pos`,
        Tm1|Tm4 -> T3 `neg`) EVERY matched entry is excitatory, so "an inhibitory entry never excites" is structurally
        true but exercised on no inhibitory synapse in that batch -- the hex patch's Pm1 (GABA) -> T3 block is where it
        is tested (`test_rectify_sign_preserved_per_entry`). The load-bearing fact on this real stream is the other
        half: the LINEAR sum flips the sign of a large fraction of those excitatory entries (presynaptic cells below
        their operating point, disinhibiting), and the rectifier removes exactly those flips."""
        W, m = self.masks(self.RECT)
        self.assertGreater(int(m.sum()), 10000)
        self.assertEqual(int((W.data[m] < 0).sum()), 0)                 # no inhibitory entry in this configuration
        ol = optic.OpticLobe(self.sub, self.r, optic.OpticParams(stream_rectify=self.RECT), device="cpu")
        self.assertEqual(int(ol.hook_info["entries_rr_matched"]), int(m.sum()))
        gen = torch.Generator().manual_seed(11); dr = (torch.rand(1, ol.n_rate, generator=gen) - 0.5) * 0.4
        y = dr[0].numpy()
        flips = int((W.data[m] * y[W.col[m]] < 0).sum())                 # linear: an excitatory entry contributing NEGATIVELY
        self.assertGreater(flips, 0.3 * int(m.sum()))
        sig = ol._stream_signals(dr)
        neg_hooked = 0
        for i, s in enumerate(ol.streams):
            blk = to_scipy(s["W_rr"]).tocoo(); x = sig[i][0].numpy()
            self.assertTrue((x >= 0).all())                              # x >= 0 under pos / neg / abs
            neg_hooked += int((blk.data * x[blk.col] < 0).sum())
        self.assertEqual(neg_hooked, 0)                                  # rectified: no entry of the stream ever flips

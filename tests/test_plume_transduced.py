...
def test_goal_only_has_no_feedback_even_after_loading_nonzero_integral():
    # feedback_gain_per_s=0.0 is the gain-0 arm; `plume:feedback=off` and `plume:feedback=0` both reach it.
    c = graph()
    a, b = [PlumeNavigation(c, feedback_gain_per_s=0.0) for _ in range(2)]
    for m in (a, b):
        m.reset(2, "cpu")
    ...
    assert a.describe()["parameters"]["variant_spec"] == "plume:feedback=off"
    for spec in ("plume:feedback=off", "plume:feedback=0"):
        m = make_instrument(c, spec)
        assert m.describe()["parameters"]["steering_integral_gain_per_s"] == 0.0
        assert m.describe()["parameters"]["variant_spec"] == "plume:feedback=off"
    assert (
        make_instrument(c, "plume:feedback=off").describe()
        == make_instrument(c, "plume:feedback=0").describe()
    )

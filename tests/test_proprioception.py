...
term off by default) and the BatchSim per-frame call, (d) the motor snapshot as checkpointed / partially-reset state.
The cache-backed class pins the real cell counts per channel and
...
class CheckpointAndResetTests(unittest.TestCase):
    """The motor snapshot is cross-frame state of the opt-in sense: a checkpoint must carry it and a partial reset
    must clear exactly the reset rows (docs/audits/proprioception_transducer.md section 6; verify:transducer)."""

    LEG_L, LEG_R, HALTERE = (12., 6., 3.), (0., 9., 15.), (20., 40., 60.)

    def sim(self, frames=4):
        """Three rows with the sense on, run `frames` frames, then given a KNOWN motor snapshot: the 22-cell graph's own
        leg MNs sit at 0 Hz in a short rollout, so the cross-frame state under test is set explicitly."""
        sim = BatchSim(3, c=graph(), device="cpu", seed=1, proprioception="all")
        for _ in range(frames): sim.step()
        sim.motor = MotorRates(leg_L=np.array(self.LEG_L), leg_R=np.array(self.LEG_R), haltere=np.array(self.HALTERE))
        return sim

    def afferent_p(self, sim):
        return sim.brain.poisson_p[:, [9, 10, 11, 12, 13, 14, 15, 17, 18, 19, 20]].clone()

    def test_checkpoint_round_trip_is_exact_for_the_next_frames_afferent_rates(self):
        a = self.sim()
        state = a.state_dict()
        self.assertIn("motor", state)
        np.testing.assert_array_equal(state["motor"].leg_L, self.LEG_L)
        b = self.sim(); b.load_state_dict(state)
        for k in ("leg_L", "leg_R", "haltere"):
            np.testing.assert_array_equal(getattr(b.motor, k), getattr(a.motor, k))
        a.step(); b.step()                                              # the frame the transducer feeds the snapshot to
        torch.testing.assert_close(self.afferent_p(b), self.afferent_p(a), rtol=0, atol=0)
        # without the snapshot the resumed frame would read tonic / load only -- the defect this pins
        c = self.sim(); c.load_state_dict(state); c.motor = None; c.step()
        self.assertGreater(float((self.afferent_p(c) - self.afferent_p(a)).abs().sum()), 0.0)
        # a pre-motor checkpoint still loads, restoring no snapshot (the old one-frame behaviour)
        d = self.sim(); d.load_state_dict({k: v for k, v in state.items() if k != "motor"})
        self.assertIsNone(d.motor)

    def test_partial_reset_clears_only_the_selected_rows(self):
        sim = self.sim()
        sim.reset(rows=[1])
        np.testing.assert_array_equal(sim.motor.leg_L, [self.LEG_L[0], 0., self.LEG_L[2]])
        np.testing.assert_array_equal(sim.motor.leg_R, [self.LEG_R[0], 0., self.LEG_R[2]])
        np.testing.assert_array_equal(sim.motor.haltere, [self.HALTERE[0], 0., self.HALTERE[2]])
        self.assertEqual(sim.body.proprio_state(sim.motor)["leg_L"].tolist(), [self.LEG_L[0], 0., self.LEG_L[2]])
        sim.step()
        dt = sim.brain.p.dt / 1000
        chord = sim.brain.poisson_p[:, [9, 10, 11, 20]].numpy()         # L, R, unsided, L (SApp23_L)
        np.testing.assert_allclose(chord[1], 10 * dt, rtol=1e-5)        # the reset row is back at tonic
        np.testing.assert_allclose(chord[0], np.array([66., 10., 38., 66.]) * dt, rtol=1e-5)   # 10 + 140 * 12/30
        np.testing.assert_allclose(chord[2], np.array([24., 80., 52., 24.]) * dt, rtol=1e-5)
        sim.reset()                                                     # a full reset drops the snapshot outright
        self.assertIsNone(sim.motor)

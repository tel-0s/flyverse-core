...
class ModuleTests(unittest.TestCase):
    ...
    def test_standalone_readout_and_id_records(self):
        """Nits: an unattached ReadoutModule raises its own shape error, not AttributeError; add_hook drops any
        captured CUDA graph as attach does; a huge selection records its size instead of every bodyId."""
        from flyverse.modules import ReadoutModule, id_list, KEEP_IDS_MAX
        loose = ReadoutModule({"cells": [0]}, lambda dt, i: {"score": i["cells"]})
        loose.reset(1, "cpu")
        with self.assertRaisesRegex(ValueError, "cover the union of reads"):
            loose.step(1., {"cells": torch.zeros(1, 1)})
        self.assertEqual(id_list(np.arange(3)), [0, 1, 2])
        self.assertEqual(id_list(np.arange(KEEP_IDS_MAX + 1)), [])
        fb = controller()
        fb._graphs["x"] = object()
        fb.add_hook(lambda f, t: None, name="h")
        self.assertEqual(fb._graphs, {})
        fb.attach(ReadoutModule({"cells": [0, 1]}, lambda dt, i: {"score": i["cells"]}, name="r"))
        record = fb.module_records()[0]
        self.assertEqual(record["n_reads"], {"cells": 2})
        self.assertEqual(len(record["reads"]["cells"]), 2)
    ...
    def test_default_path_keeps_no_drive_write_record(self):
        """B1: an optic-capable controller with nothing attached keeps no per-set_drive record.

        The branch switched the record on for every controller with an optic lobe, so `set_drive` cloned index and
        value on each call and appended to a list trimmed only by a vision frame -- unbounded on a controller that
        never calls `vision()`, and serialised into every checkpoint. Gating it on a live ExtensionRuntime makes it
        unreachable (the runtime intercepts `set_drive` through `Brain._input_target` before the record is reached),
        so the record is gone; `ExtensionRuntime` derives the same split from `_vision_has_advanced` alone.
        """
        from test_surrogate import optic_graph
        fb = FlyBrain(optic_graph(), device="cpu", lif_params=LIFParams(receptor_model=None))
        self.assertIsNotNone(fb.optic)
        for k in range(300):
            fb.brain.set_drive([3], float(k % 5))
            fb.step()
        self.assertFalse(hasattr(fb.brain, "_external_drive_writes"))
        self.assertFalse(hasattr(fb.brain, "_track_external_drive"))
        self.assertNotIn("external_drive_writes", fb.state_dict())
        self.assertAlmostEqual(float(fb.brain.drive[0, 3]), 4.)
        # attaching still holds a drive injected before the first vision frame, and drops it afterwards
        fb.add_hook(lambda f, t: None, name="watch")
        self.assertAlmostEqual(float(fb._extensions.base_drive[0, 3]), 4.)
        fb.remove_hook("watch")
        fb.vision(torch.ones(1, 4)); fb.step()
        fb.add_hook(lambda f, t: None, name="watch")
        self.assertEqual(float(fb._extensions.base_drive.abs().sum()), 0.)

    def test_readonly_module_is_numerically_neutral(self):
        """B3: `step(ms)` was split into <= 10 ms module frames whenever anything was attached, so a reads-only
        `kind='analysis'` observer changed `step(50)`. Only a module that writes a neural channel (or a vision
        encoder feeding the optic frame) may force the split."""
        c = graph().subset([2, 3, 4, 5, 6, 7])
        p = LIFParams(receptor_model=None)
        a, b = [FlyBrain(c, device="cpu", seed=11, lif_params=p) for _ in range(2)]
        seen = []
        for f in (a, b):
            f.brain.set_drive([0], 30.)
            f.stimulate([1], 150., 25.)
        a.attach(FunctionModule({"observe": [0, 1]}, {}, lambda dt, i: seen.append(dt) or {},
                                name="observe", kind="analysis"))
        self.assertFalse(a._module_frames_needed())
        for _ in range(4):
            a.step(50); b.step(50)
        self.assertEqual(seen, [50.] * 4)                      # one module frame per step(ms), not five
        for name in FlyBrain.BRAIN_TENSORS:
            torch.testing.assert_close(getattr(a.brain, name), getattr(b.brain, name), rtol=0, atol=0,
                                       msg=f"{name} moved when a reads-only module was attached")
        self.assertEqual(a.motor(), b.motor())
        # a writing module still gets the <= 10 ms frames, because its output is the next frame's input
        a.attach(FunctionModule({}, {"out": [1]}, lambda dt, i: {"out": torch.tensor([[5.]])}, name="force"))
        self.assertTrue(a._module_frames_needed())
        seen.clear(); a.step(50)

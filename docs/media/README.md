...
Device: `cuda`, NVIDIA B200, house cluster <cluster-node> (torch 2.11.0+cu128); 25 s of brain time in 74 s wall; 312 frames.
...
Device: `cuda`, NVIDIA B200, <cluster-node>; 25 s in 72 s wall; 312 frames. Wind: the demo's default 0.3 m/s blowing from the
...
## The cluster batch (four jobs, all on <cluster-node>, every one `device cuda`)

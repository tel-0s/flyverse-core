...
Two batches, 20 jobs, all on the adopted (TYPE_NT_OVERRIDE) cache (`--cache-dir /mnt/beegfs/neurome/runs/ntov-r2-e7706e/
...
(`1,1,1` makes `full`'s fast weights the adopted `sign` / `abs` ones) on `cache /mnt/beegfs/neurome/runs/r4-slowdet-f71ec1/cache`,

...
## 3. The runs (batch `cx5b-d08be3`, run dir `/mnt/beegfs/neurome/runs/cx5b-d08be3`)
...
* Provenance per draw: device `NVIDIA B200` x6; `config.receptor` model `sign` / net rule `abs` x6; backend
  ...
  `/mnt/beegfs/datasets/flyverse/cache`; both fingerprint `ef23cc27`, so the arms are the same cache.)

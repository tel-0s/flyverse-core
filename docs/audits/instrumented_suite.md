...
INTERP 10.4 rule 30 arrived with the merge of `main`: the only commit id in this audit is `a455bcf` in the
quotation above, it resolves in the post-rewrite history, and it is not an id the 2026-09-17 rewrite changed
(`docs/audits/commit_map_2026-09-17.json`). A sweep of every tracked text file for pre-rewrite ids returns
nothing.

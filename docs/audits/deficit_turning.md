...
deliver a lateralised signal (the wind, through PS230) arrives at 4 % of the inhibition it would have to overcome.**
In numbers (3 runs x 16 flies x 60 s, the 5 s start-up dropped; run means quoted):
...
1. **DNa02 is held below threshold.** Its rate-weighted synaptic input in the room is **-326 / -392 mV/s** per cell
   ...
   wiring is net *excitatory* per volley (static E +912 / I -455 mV per volley over all presynaptic types,
   `static_dna02.json`). The inversion is because the wired excitation does not fire and the wired inhibition does:
...
2. **The three lateralised excitatory classes are silent at source (section 4.3).** Compass: PFL3 **0.000 Hz** in
   every cell of every fly (24 cells, max 0.000), PFL2 0.001, and the atlas shows PFL3 is the *only* CX -> DNa02 gate
   (every other columnar / output type -- EPG, PEN, PEG, Delta7, ER, ExR1-7, FC, FS, PFL1, hDelta B-M, vDelta -- moves
   DNa02 by 0.000 at 150 Hz). Object: AOTU015 0.000 Hz (max 10, 2 of 8 cells ever), AOTU001 0.000. Motion: LLPC1,
   ...
   right DNa02-presynaptic cells above 0.5 Hz; 41 % of the LLPC1 -> DNa02 synapses come from cells that never fire.
   Six of DNa02's 30 strongest wired inputs never fire in the room (PFL3, AOTU001, LAL094, LAL179, PS034, LAL175 =
   12.1 % of |static weight|; `paths` never-firing share of DNa02's raw input 10.7 %, silent 11.5 %).
...
4. **The wind is the one lateralised sensory signal that arrives at DNa02, and it fails on dose.** Across the 48
   ...
   (`per_fly_wind_side_stats.csv`). 15 mV/s of wind-lateralised input against
   -390 mV/s of net inhibition is the localization: the JO -> PS230 -> DNa02 route (`paths` +152 mV^2 per volley,
   no silent link) is live and lateralised, and under threshold.
5. **The leg readout has drivers that fire, and they carry fixed wiring offsets, not signals.** DNge035 runs at 5.15 /
   ...
   DNge035 contralaterally within a fly (r -0.43 / -0.47 / -0.47, medians) as the atlas predicts (DNge035_R +7.10 /
   ...
   descending.
6. **What turning would require (section 7).** A lateralised drive that outweighs DNa02's tonic inhibition. The
   ...
   Tm5Y / TmY21 (`deficit_object.md`), 64 % of the DNa02-presynaptic LLPC1 cells never firing and LPT22 cancelling the
   rest, and a wind dose 25x too small. **Data-driven** next steps are measurements and calibrations: `trace` from
   T4a / T5a to LLPC1 and LPT22 (why 64 % of LLPC1 is silent; whether the LPT22 anti-correlation is the same motion
...
command against a 1 s block-shuffle null, 100 shuffles). The first 5 s are dropped. Wall time 477-484 s per run.
...
is read for the same readouts. **Floor caveat:** DNa02's null rate in the atlas is 0.000 Hz, so a purely inhibitory
input cannot register on `turn_LR` -- LT51, PS077, PS059, LAL083 / 094 / 126 reading 0.000 says nothing about them;
the atlas measures excitatory movers only.
...
LAL083 / LAL126 / PS059 -10, PS230 +10.3. Totals E +912 / I -455 mV per volley. Sign-0 input 0.8 % (LAL123 92
synapses, OA-VUMa1 88, DNg34 15).

### 4.2 Room: the input that actually arrives (`lr_DNa02_L|DNa02_R:type`, mV/s per cell, mean over 48 flies)
...
vs 0.011 Hz). The rate is set by the transients (temporal sd of the total input 250 / 325 mV/s), not by the mean.
`decompose`'s own summary agrees: `dynamic.plain.DNa02.net -325.8 / -391.5`, largest cancelling pair DNae007 +36.6 vs
...
(+8.2)** = 12.1 % of the absolute static weight; near-silent (mean < 0.02 Hz, a few cells at threshold): AOTU015
...
in-between case: 63.5 % of its 104 DNa02-presynaptic cells fire at some point, but the per-cell rollout means are 0
...
LLPC1 L-R directly within a fly (median r +0.287 / +0.308 / +0.318, range +0.04 to +0.45). The LLPC1 side asymmetry
itself (R 1.65 vs L 0.50 Hz, fly sd 0.07) is a fixed offset, not a visual one: it does not follow the wind side
(r +0.30; per run +0.19 / +0.63 / +0.16) and DNa02_R receives 1.7x the LLPC1 weight of DNa02_L (59 vs 45 entries).
...
| `turn_LR` = 0.000 (null, all three runs) | EPG, PEN_a/b, PEG, Delta7, every ER, ExR1-7, every FC / FS, PFL1, hDelta B-H / J-M, every vDelta, LT51, PS077, PS059, GNG562, LAL083 / 094 / 126 / 124 / 040 / 121 / 018, DNa13, DNge035 (the inhibitory ones are floor-limited, section 2) |
...
fires in the room; PFL3 is the sole CX gate; AOTU015 / AOTU001 (object) and the LAL / AN04B003 / PS230 interneurons
follow at 4-28 Hz per 150 Hz volley. The same two classes (LLPC1_R +9.4, PFL3_R +6.9) also move the strongest leg
driver, DNge035. The compass' columnar stages do nothing at DNa02 even at 150 Hz, so the compass route is PFL3 or
nothing.
...
* **DNa02's operating point (new).** Net input -326 / -392 mV/s from sign-correct GABA / glutamate inputs at 3-14 Hz
...
* **Motion class (T4a / T5a -> LLPC1 / HSS -> DNa02, +; LPT22 -> DNa02, -): live, lateralised, cancelled and
  under-driven.** 64 % of the DNa02-presynaptic LLPC1 cells never fire; the rest carry a signal that LPT22 removes
  at corr -0.86. **Data-driven:** `trace` from T4a / T5a to LLPC1 and LPT22 in the room (why the majority of LLPC1
...
* **Compass class (PFL3 -> DNa02, PFL3 -> LAL121 / LAL126 / LAL083): wired, functional when driven (+45 Hz at
  150 Hz; 24 Hz at 80 Hz, `interp_atlas.md` 3.3), at 0.000 Hz in the plain fly.** The ring is silent without the
...
* **Wind class (JO -> PS230 -> DNa02; JO -> GNG515 -> DNa02): live, lateralised, 25x under dose.** The wind side
...
* **Leg class (DNge035, DNa13, DNge037, DNge049, DNge073 -> leg MNs): fire, carry fixed L-R offsets (+4.4, +0.8,
  ...
  (r -0.45) and the wind side (r -0.65) weakly. Reading these DNs directly would be a readout-definition change of the
...
(wind) large enough to overcome ~350 mV/s of tonic inhibition on DNa02. The model has every route wired with the
right sign and side; it lacks the signal on three of them for documented upstream reasons and has the fourth (wind)
at 4 % of the required dose. The data-driven programme is three measurements (`trace` T4a -> LLPC1 / LPT22, `health`
...
* Atlas floor: DNa02's null is 0.000 Hz, so inhibitory movers of `turn_LR` are invisible (section 2). The atlas
  stimulates at 150 Hz for 400 ms, a dose comparison across populations of different size (`interp_atlas.md` 6.3).
* **Defects fixed this round.** (i) `scripts/interp_apply_turning.py`: the mover list built from the atlas broke on

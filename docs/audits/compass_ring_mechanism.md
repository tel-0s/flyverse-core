...
(`out/cx5/scheduler_receipt.json`), all 8 jobs on node1 (GPU ids 1-3, NVIDIA B200), finished 05:45:28-05:45:59Z. Scheduler
receipt: **8 job(s), 0 failed ({'completed': 8})**; client console (`out/cx5_cluster.log`): `8 job(s), 0 failed  (6.0 min)  run
dir /mnt/beegfs/neurome/runs/cx5-5cde5e`, `client exit 0`. Artefacts: 48 `<arm>_s<seed>.json`, 48 `.txt`, 48 `.npz`; 48 of
48 consoles say `device cuda`; every run's `provenance.execution` is `cuda` / `NVIDIA B200` / `node1`; the analysis's per-run

...
My interpreter is `C:\Users\ethee\miniconda3\python.exe` — Python 3.13.2, NumPy 2.3.5, SciPy 1.17.0,
...
`device_name NVIDIA B200`, `host node1`, `platform Linux-6.8.0-63-generic`, `torch 2.11.0+cu128`;
...
- **Infra strings: clean in code, and consistent with precedent in docs.** Zero matches for
  `beegfs|node1|athuser|datasci|sbatch|slurm|ssh|hostname|IP` in any new or changed `.py`
  (the one hit, `interp/common.py:947`, is the pre-existing `socket.gethostname()`).
  `docs/audits/banc_candidate_experiment.md:38,86,126` do name `node1`, `NVIDIA B200` and a
  `/mnt/beegfs/neurome/runs/…` path — but main already does this in `docs/audits/interp_atlas.md`,

...
**The failure line: `10 job(s), 0 failed  (5.0 min)  run dir <cluster-fs>/neurome/runs/cx6-995cd5`, `client exit 0`**;
scheduler receipt (`out/cx6/scheduler_receipt.json`, queried from the job manager itself): `10 job(s), 0 failed
({'completed': 10})`, all on **<cluster-node>**, GPU ids 0 and 1, NVIDIA B200, submitted 08:11:22Z, finished 08:14:42Z.
...
summary: >
  ...
  and 3e-179 Hz, i.e. by arithmetic underflow. THE TEST: batch cx6-995cd5 (house <cluster-node>, B200, 10 jobs, 0 failed, 40/40
...
validation:
  ...
  - "scheduler receipt 10 job(s), 0 failed ({'completed': 10}), all <cluster-node> / B200 / GPU ids 0-1; client '10 job(s), 0 failed (5.0 min) run dir <cluster-fs>/neurome/runs/cx6-995cd5', client exit 0; 40/40 json+txt+npz; 40/40 consoles 'device cuda'; analyse problems 0; the hold check (spec, factor 0, entries) passes on all 40"
...
`10 job(s), 0 failed  (5.0 min)  run dir <cluster-fs>/neurome/runs/cx6-995cd5` / `client exit 0`; the scheduler receipt
lists 10 completed jobs on <cluster-node>, GPU ids 0 and 1. `tests/test_cx_ring_structure.py` + `tests/test_cx_wedge_hold.py`:

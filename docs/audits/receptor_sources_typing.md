...
`flyverse/data/type_aliases_typing.csv`, a collision-proof copy) and `flyverse/data/type_map_typing.csv`. Raw files live
in `data/external/typing/` (git-ignored). **Round 2 (2026-09-11, task fix:typing):** the verify:tables:typing corrections
of `receptor_verification.md` are applied -- the 'none' rows of every coverage table now include the group's untyped
cells, the FlyWire annotation release is v3.1.0 (not v3.0.0), every alias row carries a `flag` column (conflict /
resolvability), the alias-tier coverage is stated with and without unresolvable rows, and the Nern Table 7 by-product is
generated by the script. Section 5 lists every number that changed.
...
|---|---|---|---|---|
| schlegel2024_Supplemental_file1_neuron_annotations.tsv | https://raw.githubusercontent.com/flyconnectome/flywire_annotations/main/supplemental_files/Supplemental_file1_neuron_annotations.tsv | flywire_annotations repo, tag **v3.1.0** = commit 8587524 (2026-07-21, "Updated FlyWire annotations matching the release of version 1.0 of the MaleCNS dataset"; extended with Berg et al. 2025); FlyWire release 783. Round 1 mislabelled this v3.0.0: tag v3.0.0 is commit a92610e (2025-10-09), whose SD1 (sha256 533db093..., 32,638,576 B) has 8,806 cell types, 4,626 MaleCNS names verbatim and 1,799 root_ids typed differently from v3.1.0 | 31,718,505 | 9a4f8b2f843196074431ebd7cd883536afa1be86c8a4ce90970441e8be81d1be |
...
* Schlegel P, Yin Y, Bates AS, et al. (2024) Whole-brain annotation and multi-connectome cell typing of *Drosophila*.
  ...
  github.com/flyconnectome/flywire_annotations (v3.x extends them with Berg et al. 2025 bioRxiv: `synonyms`,
  `fru_dsx`, `dimorphism`, `supertype`, and optic types renamed to the Matsliah et al. 2024 convention -- the renames
  Tm5Y -> Tm5f, LPi12 -> LPi14 are already in v3.0.0; the files used here are v3.1.0, commit 8587524, 2026-07-21).
...
* FlyWire names: SD1 v3.1.0 has 8,840 `cell_type` values; 4,671 MaleCNS type names occur in it verbatim (v3.0.0:
  8,806 types, 4,626 names; the 2024 paper release: 5,634 types, 1,708 MaleCNS names). Optic-lobe types were renamed
  between the paper release and v3 (Tm5Y -> Tm5f, LPi12 -> LPi14, Tm6 1,297 -> 1 cell), and MaleCNS `flywireType`
  carries the v3 / Matsliah name.
...
* Name collisions (a MaleCNS type name exists in the source, but the MaleCNS authors' own column maps the type to a
  different name): 182 types vs FlyWire v3.1.0, 18 vs hemibrain. 48 of the FlyWire ones are splits (`AOTU050`
  ...
  `NAME COLLISION` note. For 6 optic types (LPi12, LT82a, LT82b, Tm12, Tm6, TmY18) the collision is a release
  artefact: the name exists in the 2024 paper release and Nern Table 7 confirms it, while v3.1.0 reuses the name for a
  different Matsliah-convention type; these get tier exact with an explicit CAUTION in the evidence and the flag
  `paper_release_only;v3_name_collision`. (Round 1 listed 9 such types; Li12 / Li16 / Li19 exist in v3.0.0 and v3.1.0
  with 41-42 / 36 / 4 cells and are ordinary exact matches.) 10 exact FlyWire rows in all rest on the 2024 paper
  release (flag `paper_release_only`): the 6 above plus LPi21, Pm1, Pm4, Tm5Y, whose names are absent from v3
  altogether.
...
Row counts: `type_aliases.csv` 22,200 data rows (= `type_aliases_typing.csv`), `type_map_typing.csv` 22,187 (the 13
`(unmatched)` rows dropped); round 1 quoted 22,201 / 22,188 by counting the column-header line.

**`flag` column (round 2).** Each row carries a `;`-joined caveat list (empty = none). The tier is unchanged; a
downstream join that treats tier `alias` as trustworthy must drop rows flagged `unresolvable` / `unmatched` /
`conflict_nern7_vs_malecns`.

| flag | meaning | rows (system / tier) |
|---|---|---|
| unresolvable | FlyWire alias absent from SD1 v3.1.0 *and* from the 2024 paper release; hemibrain alias absent from SD5 / SD1 `hemibrain_type` (bare body ids 1343403608, 5901212906; AOTU002, AVL006, AVLP299, FS, KCab, mAL5; five strings truncated at `(`: PEN_a(PEN1, PEN_b(PEN2, PS279(hb1499087543, PVLP120(hb1599285725, SLP405_a(hb5813008928) | 710 (flywire/alias 697, hemibrain/alias 13) |
| cb_placeholder | FlyWire alias is a `CB####` / `CB.FB*` placeholder absent from v3.1.0 (244 of them absent from both releases, 103 present in the 2024 paper release) | 347 (flywire/alias) |
| paper_release_only | FlyWire name present only in the 2024 paper release of SD1 (renamed or dropped in v3.1.0); resolvable with that release | 203 (flywire/alias 193, flywire/exact 10) |
| unmatched | the `(unmatched)` Nern Table 7 rows | 13 (flywire/alias) |
| conflict_nern7_vs_malecns | FlyWire alias rows of a type whose Nern Table 7 FlyWire name and MaleCNS majority `flywireType` name **different types**: the Cm -> Sm off-by-one series (Cm20 Sm17 vs Sm18, Cm29 Sm37 vs Sm36, Cm30-35), the Li shifts (Li31/32/33/38/39), AOTU056 CB1558 vs CB2216, AOTU058, CL357, SMP217, Tm40 CB3851 vs Tm40, LoVP19 LTe49a,b,d,e,f vs LC46, LoVP90a-c LT42x vs LTe42x, LT82a/b vs LT82 | 55 rows, 25 types (flywire/alias) |
| conflict_nern7_vs_malecns_notation | the two strings differ but are the same names written differently (`LTe49a,b,d,e,f` vs `LTe49a,LTe49b,...`, `Sm12+Sm18` vs `Sm12`, `VS1+...+VS8`) or a parent / subtype split (`MeTu2` vs `MeTu2a`, `MeTu4` vs `MeTu4a-f`, TmY19a, Cm8, 5thsLNv_LNd6, LC31b, LoVP20-24, LoVP27) | 76 rows, 22 types (flywire/alias) |
| minority_agreement | exact row whose MaleCNS cross-dataset column names the type on fewer than half of the cells (KCa'b'-ap1 2/199 vs KCapbp-ap1; AOTU054 1/3 vs CB1558; CB2572 1/9 vs CB2572a/b; ...) | 28 (flywire/exact 18, hemibrain/exact 10) |
| merged_list_only | exact row whose MaleCNS column carries the name only inside a merged comma list (the 38 FlyWire types hold 157 cells) | 86 (flywire/exact 38, hemibrain/exact 48) |
| name_only | exact row resting on the name alone (MaleCNS column empty on every cell): FlyWire AN01A021, JO-unclear; 21 hemibrain types | 23 (flywire/exact 2, hemibrain/exact 21) |
| v3_name_collision | paper-release exact row whose name denotes a different type in v3.1.0 (the 6 CAUTION rows) | 6 (flywire/exact) |

The 47 string mismatches between Nern Table 7 and MaleCNS `flywireType` are the verification record's 47 (719 types
compared: majority `flywireType` string equal to neither `Schlegel_type` nor `Matsliah_type`); the split into 25
conflicts + 22 notation differences is this round's sub-classification (name sets disjoint and not a parent /
subtype pair = conflict). Round 1's rule for exact rows ('the MaleCNS column does not contradict the name') admits the
minority_agreement / merged_list_only / name_only rows; they keep tier exact and are flagged, not demoted. MANC
'exact' (3,433 types) is `mancType == type` from the MaleCNS column alone -- there is no independent MANC type list.
...
## 4. Coverage (output of `scripts/build_type_map_typing.py`, round 2)

Per-type best tiers are in `out/type_map_typing_per_type.csv` (git-ignored; columns `flywire_resolvable`,
`hemibrain_resolvable`, `any_resolvable`, `any_strict`, `flywire_strict` added in round 2). "Group optic" is superclass
`ol_intrinsic` + `visual_projection` + `ol_sensory` as the task defines it; visual centrifugal cells are separate.
**Every 'none' row now includes the group's untyped cells** (type count 0 for them), so the rows of every table sum to
its denominator; the exact / alias / fuzzy / class rows are unchanged from round 1. Percentages are of the denominator
row (all cells / all output synapses of the group, untyped included).

MaleCNS node set: 167,106 cells, 121,427,136 output synapses with a nonzero sign (abs W), 124,025,046 raw output synapses; typed cells 164,501 (98.4%) in 11,751 types; untyped 2,605 cells (1.6%) carrying 900,071 abs-W / 961,116 raw output synapses (0.77%) -- untyped cells are counted in the 'none' row of every table (type count 0) and in the denominators; the rows of every table sum to its denominator.
...
| none | 3,700 (31.5%) | 25,047 (15.0%) | 22,552,976 (18.6%) | 22,964,514 (18.5%) |
...
| none | 3,502 (29.8%) | 18,826 (11.3%) | 19,199,552 (15.8%) | 19,527,613 (15.7%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |
...
| none | 412 (3.5%) | 4,952 (3.0%) | 3,165,291 (2.6%) | 3,321,550 (2.7%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |
...
| none | 3,792 (32.3%) | 25,671 (15.4%) | 23,043,204 (19.0%) | 23,463,818 (18.9%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |
...
| none | 6,804 (57.9%) | 131,965 (79.0%) | 81,591,880 (67.2%) | 82,416,920 (66.5%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |
...
| none | 11,019 (93.8%) | 62,836 (37.6%) | 67,648,912 (55.7%) | 69,885,073 (56.3%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |
...
| none | 7,569 (64.4%) | 144,314 (86.4%) | 93,843,096 (77.3%) | 95,943,850 (77.4%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |

### Group optic (superclasses: ['ol_intrinsic', 'ol_sensory', 'visual_projection']), best tier over the three systems; 37 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 592 (94.3%) | 103,600 (99.0%) | 51,752,132 (99.7%) | 51,682,161 (99.7%) |
| alias | 5 (0.8%) | 863 (0.8%) | 122,979 (0.2%) | 122,972 (0.2%) |
| none | 31 (4.9%) | 228 (0.2%) | 48,342 (0.1%) | 49,231 (0.1%) |
...
| none | 39 (6.2%) | 375 (0.4%) | 137,435 (0.3%) | 138,324 (0.3%) |
| total (denominator) | 628 | 104,691 | 51,923,456 | 51,854,364 |
...
| none | 341 (54.3%) | 95,921 (91.6%) | 44,026,792 (84.8%) | 43,942,903 (84.7%) |
| total (denominator) | 628 | 104,691 | 51,923,456 | 51,854,364 |
...
| none | 45 (7.2%) | 1,111 (1.1%) | 213,357 (0.4%) | 214,239 (0.4%) |
| total (denominator) | 628 | 104,691 | 51,923,456 | 51,854,364 |

### Group visual_centrifugal (superclasses: ['visual_centrifugal']), best tier over the three systems; 1 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 105 (97.2%) | 559 (99.3%) | 1,759,823 (99.9%) | 2,137,132 (99.9%) |
| none | 3 (2.8%) | 4 (0.7%) | 2,605 (0.1%) | 2,605 (0.1%) |
...
| none | 39 (36.1%) | 238 (42.3%) | 493,482 (28.0%) | 521,986 (24.4%) |
| total (denominator) | 108 | 563 | 1,762,428 | 2,139,737 |
...
| none | 7 (6.5%) | 30 (5.3%) | 4,430 (0.3%) | 4,519 (0.2%) |
| total (denominator) | 108 | 563 | 1,762,428 | 2,139,737 |

### Group central (superclasses: ['cb_intrinsic']), best tier over the three systems; 880 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 4,872 (73.8%) | 25,226 (78.4%) | 28,834,388 (77.0%) | 30,215,363 (77.2%) |
| alias | 1,366 (20.7%) | 4,171 (13.0%) | 5,962,815 (15.9%) | 6,165,053 (15.7%) |
| none | 367 (5.6%) | 2,763 (8.6%) | 2,669,703 (7.1%) | 2,770,300 (7.1%) |
...
| fuzzy | 1 (0.0%) | 2 (0.0%) | 3,236 (0.0%) | 3,236 (0.0%) |
| none | 448 (6.8%) | 3,224 (10.0%) | 3,070,837 (8.2%) | 3,180,311 (8.1%) |
| total (denominator) | 6,605 | 32,160 | 37,466,908 | 39,150,716 |
...
| none | 2,260 (34.2%) | 9,315 (29.0%) | 9,950,438 (26.6%) | 10,300,779 (26.3%) |
| total (denominator) | 6,605 | 32,160 | 37,466,908 | 39,150,716 |
...
| none | 6,565 (99.4%) | 32,030 (99.6%) | 37,231,268 (99.4%) | 38,882,008 (99.3%) |
| total (denominator) | 6,605 | 32,160 | 37,466,908 | 39,150,716 |

### Group descending (superclasses: ['descending_neuron']), best tier over the three systems; 4 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 445 (92.7%) | 1,182 (90.0%) | 3,696,659 (94.7%) | 3,882,984 (94.9%) |
| alias | 32 (6.7%) | 122 (9.3%) | 141,951 (3.6%) | 141,951 (3.5%) |
| none | 3 (0.6%) | 10 (0.8%) | 63,105 (1.6%) | 67,357 (1.6%) |
...
| none | 308 (64.2%) | 903 (68.7%) | 2,319,620 (59.5%) | 2,435,461 (59.5%) |
| total (denominator) | 480 | 1,314 | 3,901,715 | 4,092,292 |
...
| none | 4 (0.8%) | 12 (0.9%) | 20,985 (0.5%) | 35,577 (0.9%) |
| total (denominator) | 480 | 1,314 | 3,901,715 | 4,092,292 |

### Group ascending (superclasses: ['ascending_neuron']), best tier over the three systems; 5 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 4 (0.7%) | 19 (1.0%) | 7,499 (0.1%) | 24,026 (0.5%) |
| alias | 410 (72.8%) | 1,324 (70.8%) | 4,320,317 (84.2%) | 4,340,360 (83.7%) |
| none | 149 (26.5%) | 527 (28.2%) | 804,688 (15.7%) | 818,299 (15.8%) |
...
| none | 557 (98.9%) | 1,858 (99.4%) | 5,016,557 (97.7%) | 5,066,738 (97.8%) |
| total (denominator) | 563 | 1,870 | 5,132,504 | 5,182,685 |
...
| none | 2 (0.4%) | 12 (0.6%) | 13,633 (0.3%) | 13,633 (0.3%) |
| total (denominator) | 563 | 1,870 | 5,132,504 | 5,182,685 |

### Group vnc (superclasses: ['vnc_efferent', 'vnc_endocrine', 'vnc_intrinsic', 'vnc_motor', 'vnc_sensory']), best tier over the three systems; 980 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| alias | 1 (0.0%) | 27 (0.1%) | 12,809 (0.1%) | 12,809 (0.1%) |
| none | 3,118 (100.0%) | 20,292 (99.9%) | 18,580,568 (99.9%) | 18,859,175 (99.9%) |
...
| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| none | 3,119 (100.0%) | 20,319 (100.0%) | 18,593,376 (100.0%) | 18,871,984 (100.0%) |
| total (denominator) | 3,119 | 20,319 | 18,593,376 | 18,871,984 |
...
| none | 8 (0.3%) | 1,095 (5.4%) | 307,837 (1.7%) | 349,599 (1.9%) |
| total (denominator) | 3,119 | 20,319 | 18,593,376 | 18,871,984 |
...
### Group sensory_motor_other (superclasses: ['cb_efferent', 'cb_endocrine', 'cb_motor', 'cb_sensory', 'efferent_ascending', 'efferent_descending', 'sensory_ascending', 'sensory_descending']), best tier over the three systems; 135 untyped cells in the denominator and the 'none' row

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 160 (64.5%) | 4,613 (82.0%) | 2,141,239 (81.1%) | 2,159,160 (79.3%) |
| alias | 59 (23.8%) | 353 (6.3%) | 121,550 (4.6%) | 176,561 (6.5%) |
| none | 29 (11.7%) | 660 (11.7%) | 376,212 (14.3%) | 386,803 (14.2%) |
...
| none | 32 (12.9%) | 676 (12.0%) | 376,212 (14.3%) | 387,003 (14.2%) |
| total (denominator) | 248 | 5,626 | 2,639,001 | 2,722,524 |
...
| none | 180 (72.6%) | 2,848 (50.6%) | 1,183,860 (44.9%) | 1,266,325 (46.5%) |
| total (denominator) | 248 | 5,626 | 2,639,001 | 2,722,524 |
...
| none | 214 (86.3%) | 5,218 (92.7%) | 2,340,096 (88.7%) | 2,389,480 (87.8%) |
| total (denominator) | 248 | 5,626 | 2,639,001 | 2,722,524 |
...
### Flag counts (rows; a row can carry several flags)

| flag | rows | systems / tiers |
|---|---|---|
| unresolvable | 710 | flywire/alias 697, hemibrain/alias 13 |
| cb_placeholder | 347 | flywire/alias 347 |
| paper_release_only | 203 | flywire/alias 193, flywire/exact 10 |
| merged_list_only | 86 | flywire/exact 38, hemibrain/exact 48 |
| conflict_nern7_vs_malecns_notation | 76 | flywire/alias 76 |
| conflict_nern7_vs_malecns | 55 | flywire/alias 55 |
| minority_agreement | 28 | flywire/exact 18, hemibrain/exact 10 |
| name_only | 23 | flywire/exact 2, hemibrain/exact 21 |
| unmatched | 13 | flywire/alias 13 |
| v3_name_collision | 6 | flywire/exact 6 |

### Resolvability of the alias tier

* FlyWire alias rows: 4,589; 903 name a type absent from Schlegel SD1 v3.1.0 cell_type (13 '(unmatched)' rows included; 776 carry the 'name not in ... cell_type list' note in `evidence`, the Nern-Table-7-derived rows do not). Of the absent names 347 are 'CB' placeholders (flag `cb_placeholder`; 244 of them in neither release), 193 exist only in the 2024 paper release (flag `paper_release_only`, resolvable with that release), and 697 are in neither release (flag `unresolvable`).
* hemibrain alias rows: 1,210; 13 name a type absent from SD5 / SD1 hemibrain_type (1343403608, 5901212906, AOTU002, AVL006, AVLP299, FS, KCab, PEN_a(PEN1, PEN_b(PEN2, PS279(hb1499087543, PVLP120(hb1599285725, SLP405_a(hb5813008928, mAL5) -> flag `unresolvable`.
* Nern Table 7 FlyWire name vs MaleCNS majority flywireType: 719 types compared, 47 differ as strings; 25 name different types (flag `conflict_nern7_vs_malecns`: Tm40 T7=['CB3851'] MaleCNS=Tm40, Cm20 T7=['Sm17'] MaleCNS=Sm18, Cm29 T7=['Sm37'] MaleCNS=Sm36, Cm30 T7=['CB3838', 'Sm39'] MaleCNS=Sm38, Cm31a T7=['CB3841', 'Sm41'] MaleCNS=Sm40, Cm31b T7=['Sm42'] MaleCNS=Sm41, Cm32 T7=['CB3844', 'Sm36'] MaleCNS=Sm35, Cm33 T7=['CB3838', 'Sm38'] MaleCNS=Sm37, Cm34 T7=['Sm43'] MaleCNS=Sm42, Cm35 T7=['CB3843', 'Sm40'] MaleCNS=Sm39, Li31 T7=['LLpt101_L56-Lp2'] MaleCNS=Li29, Li32 T7=['Li31'] MaleCNS=Li32, Li33 T7=['1343403608', 'Li29'] MaleCNS=Li30, Li38 T7=['mALC1', 'Li30'] MaleCNS=Li31, Li39 T7=['mALC2', 'Li32'] MaleCNS=Li33, AOTU056 T7=['CB1558'] MaleCNS=CB2216, AOTU058 T7=['CB1329'] MaleCNS=AOTU058, CL357 T7=['SMPp&v1B_H01'] MaleCNS=CL357, LoVP19 T7=['LTe49a,b,d,e,f'] MaleCNS=LC46, LoVP90a T7=['LT42a'] MaleCNS=LTe42a, LoVP90b T7=['LT42b'] MaleCNS=LTe42b, LoVP90c T7=['LT42c'] MaleCNS=LTe42c, LT82a T7=['LT82a'] MaleCNS=LT82, LT82b T7=['LT82b'] MaleCNS=LT82, SMP217 T7=['CB1791'] MaleCNS=SMP217); 22 are notation / parent-subtype differences (flag `conflict_nern7_vs_malecns_notation`: TmY19a, Cm8, 5thsLNv_LNd6, LC31b, LoVP20, LoVP21, LoVP22, LoVP23, LoVP24, LoVP27, MeTu2a, MeTu2b, MeTu3a, MeTu3b, MeTu3c, MeTu4a, MeTu4b, MeTu4c, MeTu4d, MeTu4e, MeTu4f, VS).
* Exact-row caveats (FlyWire / hemibrain): minority_agreement 18 / 10 types, merged_list_only 38 / 48, name_only 2 / 21, paper_release_only 10 (of which v3_name_collision 6); the merged_list_only FlyWire types hold 157 cells. MANC 'exact' (3,433 types) is mancType == type from the MaleCNS column alone (no independent MANC type list).
* FlyWire alias-tier types: 3,465; 448 of them (2,052 cells, 1,834,535 raw out-syn) have no alias resolvable in either SD1 release. Best-of-three alias-tier types: 1,873; 86 (339 cells, 216,323 raw) owe the tier solely to unresolvable names and drop to 'none' when those rows are excluded. Under the strict rule (also excluding paper-release-only names and the 25 Nern-vs-MaleCNS conflicts) 94 best-of-three alias-tier types (409 cells, 270,047 raw) drop to 'none'; in the FlyWire system alone 547 alias-tier types (2,831 cells, 3,012,540 raw) drop to 'none' and the 10 paper-release exact types (LPi12, LPi21, LT82a, LT82b, Pm1, Pm4, Tm12, Tm5Y, Tm6, TmY18; 5,184 cells) fall back to their MaleCNS-flywireType alias row (their optic_inventory exact row keeps the best-of-three tier at exact).

### Whole CNS, best tier over the three systems, unresolvable rows excluded (flags unresolvable / unmatched dropped; paper-release-only names kept)

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 6,178 (52.6%) | 135,199 (80.9%) | 88,191,744 (72.6%) | 90,100,826 (72.6%) |
| alias | 1,787 (15.2%) | 6,521 (3.9%) | 10,482,839 (8.6%) | 10,743,383 (8.7%) |
| none | 3,786 (32.2%) | 25,386 (15.2%) | 22,752,556 (18.7%) | 23,180,837 (18.7%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |

### Whole CNS, best tier over the three systems, strict: unresolvable, paper-release-only and Nern-vs-MaleCNS conflict rows excluded

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 6,178 (52.6%) | 135,199 (80.9%) | 88,191,744 (72.6%) | 90,100,826 (72.6%) |
| alias | 1,779 (15.1%) | 6,451 (3.9%) | 10,429,136 (8.6%) | 10,689,659 (8.6%) |
| none | 3,794 (32.3%) | 25,456 (15.2%) | 22,806,260 (18.8%) | 23,234,561 (18.7%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |

### Whole CNS, system flywire, unresolvable rows excluded

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 4,493 (38.2%) | 99,998 (59.8%) | 65,631,276 (54.0%) | 67,310,653 (54.3%) |
| alias | 3,017 (25.7%) | 39,383 (23.6%) | 31,014,316 (25.5%) | 31,412,804 (25.3%) |
| fuzzy | 1 (0.0%) | 2 (0.0%) | 3,236 (0.0%) | 3,236 (0.0%) |
| none | 4,240 (36.1%) | 27,723 (16.6%) | 24,778,308 (20.4%) | 25,298,353 (20.4%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |

### Whole CNS, system hemibrain, unresolvable rows excluded

| tier | types | cells | out synapses (abs W) | out synapses (raw) |
|---|---|---|---|---|
| exact | 3,899 (33.2%) | 27,998 (16.8%) | 31,614,696 (26.0%) | 33,011,092 (26.6%) |
| alias | 906 (7.7%) | 6,640 (4.0%) | 7,367,022 (6.1%) | 7,698,584 (6.2%) |
| fuzzy | 137 (1.2%) | 488 (0.3%) | 774,065 (0.6%) | 818,971 (0.7%) |
| none | 6,809 (57.9%) | 131,980 (79.0%) | 81,671,360 (67.3%) | 82,496,399 (66.5%) |
| total (denominator) | 11,751 | 167,106 | 121,427,136 | 124,025,046 |

### By-product: Nern 2025 Table 7 transmitter predictions, optic lobe (male) vs FlyWire (female), where both are called

Rows with an OL_type (732; the OL_type-less LTe12 row is excluded) and a call on both sides: 544; agree 503; disagree 41; a further 173 rows have `unclear` on one side and a call on the other (179 rows have `unclear` on at least one side: OL unclear 172, FW unclear 9, 2 both unclear, 4 unclear against a blank FW cell; 179 - 4 = 175 is the verification record's count, which keeps the 2 both-unclear rows; FW blank 13). The systematic pattern is glutamate (OL) vs GABA (FW) in Dm / Pm / Cm / LPi / Li types. MaleCNS model NT (`nt` column of cache/neurons.parquet, majority per type) added for reference.
...
wrote flyverse/data/type_aliases.csv (= type_aliases_typing.csv; 22,200 data rows) and flyverse/data/type_map_typing.csv (22,187 data rows)
...
|---|---|---|
| whole CNS, best of three, none | 22,442 cells (13.4 %) / 22,003,398 raw (17.7 %) | **25,047 (15.0 %) / 22,964,514 (18.5 %)**; abs-W 22,552,976 (18.6 %) |
| whole CNS incl. class, none | 16,221 (9.7 %) / 18,566,497 (15.0 %) | **18,826 (11.3 %) / 19,527,613 (15.7 %)** |
| whole CNS incl. MANC, none | 2,347 (1.4 %) / 2,360,434 (1.9 %) | **4,952 (3.0 %) / 3,321,550 (2.7 %)** |
| whole CNS, system flywire, none | 23,066 (13.8 %) / 22,502,702 (18.1 %) | **25,671 (15.4 %) / 23,463,818 (18.9 %)** |
| whole CNS, system hemibrain, none | 129,360 (77.4 %) / 81,455,804 (65.7 %) | **131,965 (79.0 %) / 82,416,920 (66.5 %)** (66.45 %) |
| whole CNS, system optic_inventory, none | 60,231 (36.0 %) / 68,923,957 (55.6 %) | 62,836 (37.6 %) / 69,885,073 (56.3 %) |
| whole CNS, system manc, none | 141,709 (84.8 %) / 94,982,734 (76.6 %) | 144,314 (86.4 %) / 95,943,850 (77.4 %) |
| group central, none | 1,883 (5.9 %) / 2,147,694 (5.5 %) | **2,763 (8.6 %) / 2,770,300 (7.1 %)** |
| group vnc, system manc / incl. MANC, none | 115 (0.6 %) / 77,649 (0.4 %) | **1,095 (5.4 %) / 349,599 (1.9 %)** |
| group optic, none | 191 (0.2 %) / 47,850 (0.1 %) | 228 (0.2 %) / 49,231 (0.1 %) |
| every other group / system 'none' row | typed cells only | + the group's untyped cells (optic 37, visual_centrifugal 1, central 880, descending 4, ascending 5, vnc 980, sensory_motor_other 135; 563 untyped cells have no group) |
| FlyWire annotation release | 'SD1 v3.0.0' in CSV headers, evidence strings, docstring, doc | **flywire_annotations v3.1.0 (commit 8587524, 2026-07-21)** everywhere; v3.0.0 = commit a92610e (2025-10-09), 8,806 types |
| alias tables | no conflict / resolvability column | `flag` column (definitions in section 3): unresolvable 710, cb_placeholder 347, paper_release_only 203, unmatched 13, conflict_nern7_vs_malecns 55 rows / 25 types, _notation 76 / 22, minority_agreement 28, merged_list_only 86, name_only 23, v3_name_collision 6 |
| FlyWire alias rows naming a type absent from SD1 v3.1.0 | not stated | 903 of 4,589 (347 CB placeholders, 193 paper-release-only, 697 in neither release, 13 unmatched); 776 carried the 'name not in' evidence note, the Nern-Table-7-derived rows did not |
| alias-tier coverage with unresolvable rows excluded | not stated | FlyWire system: alias 3,465 -> 3,017 types (41,435 -> 39,383 cells, 26.8 % -> 25.3 % raw); 448 alias-tier types (2,052 cells, 1,834,535 raw) have no resolvable alias. Best of three: alias 1,873 -> 1,787 types (6,860 -> 6,521 cells, 8.8 % -> 8.7 % raw), none 25,047 -> 25,386 cells; 86 types (339 cells, 216,323 raw) lose the tier (84 / 333 / 210,660 from FlyWire rows alone -- the record's figures -- plus mAL5A2 and AVL006_a from the 13 hemibrain rows). Strict (also dropping paper-release-only names and the 25 conflicts): alias 1,779 types / 6,451 cells (8.6 % raw); FlyWire system alias 2,926 types (547 alias-tier types drop to none, the 10 paper-release exact types fall back to their MaleCNS-flywireType alias row) |
| hemibrain alias rows naming absent types | not stated | 13 of 1,210, flagged unresolvable; hemibrain alias 911 -> 906 types when excluded |
| exact rows agreeing on < half their cells / name only inside a merged list | not stated | 28 (18 FlyWire + 10 hemibrain) / 86 (38 FlyWire = 157 cells, 48 hemibrain); flagged, tier kept |
| Nern Table 7 by-product | 545 double-called, 503 agree, 42 disagree; 179 with unclear on one side | **544 / 503 / 41** (the OL_type-less LTe12 row dropped); 179 rows have unclear on at least one side, 175 excluding the 4 whose FW cell is blank (the record's count), **173** with a call on the other side (2 rows are unclear on both sides) |
| row counts | 22,201 / 22,188 | **22,200 / 22,187** data rows |
| optic types with CAUTION exact rows | 9 (LPi12, LT82a, LT82b, Li12, Li16, Li19, Tm12, Tm6, TmY18) | **6** (LPi12, LT82a, LT82b, Tm12, Tm6, TmY18); Li12 / Li16 / Li19 are ordinary exact matches; 10 exact rows rest on the paper release, 4 (LPi21, Pm1, Pm4, Tm5Y) for names absent from v3 |
| tier row counts by system | unchanged | unchanged (flywire 4,493 / 4,589 / 1; hemibrain 3,899 / 1,210 / 154; hemibrain_body 123; optic_inventory 732; manc 3,433 / 853; literature 1,679; malecns_class 1,034) |

Not changed: the per-type tier assignment (`type_map_typing.csv` rows are the round-1 rows plus the `flag` column and
the v3.1.0 wording in `evidence`), the exact / alias / fuzzy / class rows of every coverage table, and the name-collision
counts (182 FlyWire, 18 hemibrain).

...
after each stage against a recorded golden, verified equal to the tree that predates
this whole surface. **Regenerating that golden is an owner decision, not a test fix.**

## Decided open questions

`docs/EXTENSIBILITY_SPEC.md` left three questions open. All three are **decided and
owner-accepted**; the implementation below follows them and they are not open for
per-experiment reinterpretation:

| question | decision |
|---|---|
| How does a module's Poisson write combine with sensory forcing and timed pulses? | By **maximum**, exactly as `stimulate` already combines with the senses. It is never additive, and a negative requested rate clamps to zero. Drive writes, by contrast, are additive and may be negative. |
| What identifies a synthetic cell? | A unique **negative int64** bodyId under `dataset="synthetic"`. Biological IDs never change, and no biological-to-biological edge is ever rewritten. |
| What state does a module's `step` see? | The **previous frame's** state. Every module's inputs are gathered before any module runs, so attachment order cannot create a zero-delay feedback loop. |
...
One boundary case: a `set_drive` issued **before** the first `attach`/`add_hook` is
held from then on only if no optic frame has run since. After a vision frame the whole
drive tensor belongs to the optic lobe (the frame overwrites it, it does not
accumulate), so such a write is treated as sensory and the next vision frame replaces
it — exactly as it would with no extensions attached. Re-issue the drive after
attaching if it is meant to be held.
...
a zero-delay feedback loop. When an attached module **writes** a neural channel — or
is a vision encoder feeding the optic frame — long `step(ms)` calls split into frames
of at most 10 ms, because that module's output is the next frame's input; the final
shorter frame uses its actual duration. `step_budget` can use shorter frames too.
Fractional LIF time still carries over. Hooks remain once per API call.

A module that writes nothing — a `ReadoutModule`, any `kind="analysis"` observer, a
`boundary="motor"` decoder the body evaluates — is **numerically neutral**: it does
not split the call, it is stepped once per `step(ms)`, and `step(50)` is bit-identical
to the same call with nothing attached (`tests/test_modules.py::test_readonly_module_is_numerically_neutral`).
Observation must not perturb the model. The same holds for hooks alone, which run
once around the call either way. Without extensions, `step(ms)` retains its existing
...
The differentiable optic frame is the **same** substep as inference: it goes through
`OpticLobe._recurrent` / `_output`, so the opt-in per-stream hooks
(`OpticParams.stream_rectify` / `stream_adapt` / `spatial_suppress`, see
[optic stream hooks](audits/optic_stream_hooks.md)) act on it exactly as they do on
the shipped path, and with the hooks off the two paths evaluate identical expressions.
Both are pinned bit for bit in `tests/test_surrogate.py`; a gradient path that quietly
dropped an active hook would be a different forward model, not an approximation.

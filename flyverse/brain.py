...
class Brain:
    ...
    def set_drive(self, idx, mv) -> None:
        ...
        if getattr(self, "_track_external_drive", False):
            self._external_drive_writes.append((self._idx(idx).clone(), value.clone()))
    ...
    def reset(self, rows=None) -> None:
        ...
        if self.p.surrogate_grad:
            self.detach_state()
        if rows is None and hasattr(self, "_external_drive_writes"):
            self._external_drive_writes.clear()

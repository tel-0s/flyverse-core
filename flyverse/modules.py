...
class SNNModule(FunctionModule):
    ...
    def describe(self):
        ...
        return {"name": self.name, "class": identifier(type(self)), "kind": self.kind, "trainable": False,
                ...
                "input_cells": {k: self.c.neurons.bodyId.to_numpy()[idx].tolist() for k, idx in self.input_cells.items()},
                "output_cells": {k: self.c.neurons.bodyId.to_numpy()[idx].tolist() for k, idx in self.output_cells.items()},
...
class ExtensionRuntime:
    ...
    def __init__(self, fb):
        ...
        if getattr(fb, "_vision_has_advanced", False):
            self.base_drive.zero_()
            for idx, value in fb.brain._external_drive_writes:
                self.base_drive[:, idx] = value
    ...
    def records(self):
        ...
        return [{**snapshot(m.describe()), "quantity_in": m.quantity_in, "channel_out": m.channel_out,
                 ...
                 "reads": {k: ids[v].tolist() for k, v in self.bindings[name][0].items()},
                 "writes": {k: ids[v].tolist() for k, v in self.bindings[name][1].items()}}
